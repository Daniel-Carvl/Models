...
# =============================================================================
# PROJEÇÃO FUTURA - Trips palmi
# =============================================================================

# Suponha que você tenha os dados futuros em uma pasta "WorldClim/Future"
future_folder <- "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6"
future_files <- list.files(future_folder, pattern = "\\.tif$", full.names = TRUE)

future_files <- c(
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_HadGEM3-GC31-LL_ssp245_2041-2060.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_HadGEM3-GC31-LL_ssp245_2081-2100.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_HadGEM3-GC31-LL_ssp585_2041-2060.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_HadGEM3-GC31-LL_ssp585_2081-2100.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_MIROC6_ssp245_2041-2060.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_MIROC6_ssp245_2081-2100.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_MIROC6_ssp585_2041-2060.tif",
  "C:/Users/Usuário/Documents/Daniel/Artigo Adriano/Dados_Futuros_CMIP6/wc2.1_5m_bioc_MIROC6_ssp585_2081-2100.tif"
)

# Criar tabela de metadados dos cenários
scenarios_metadata <- data.frame(
  file_path = future_files,
  model = c("HadGEM3-GC31-LL", "HadGEM3-GC31-LL", "HadGEM3-GC31-LL", "HadGEM3-GC31-LL",
            "MIROC6", "MIROC6", "MIROC6", "MIROC6"),
  ssp = c("245", "245", "585", "585", "245", "245", "585", "585"),
  period = c("2041-2060", "2081-2100", "2041-2060", "2081-2100",
             "2041-2060", "2081-2100", "2041-2060", "2081-2100"),
  scenario_name = c("HadGEM3-GC31-LL_ssp245_2041-2060", "HadGEM3-GC31-LL_ssp245_2081-2100",
                    "HadGEM3-GC31-LL_ssp585_2041-2060", "HadGEM3-GC31-LL_ssp585_2081-2100",
                    "MIROC6_ssp245_2041-2060", "MIROC6_ssp245_2081-2100",
                    "MIROC6_ssp585_2041-2060", "MIROC6_ssp585_2081-2100")
)

print(scenarios_metadata)

# 2. CARREGAR METADADOS DA PCA E THRESHOLD ------------------------------------

pca_metadata <- readRDS("pca_pc1_pc2_metadata.rds")
thresholds <- sdm::threshold(my.sdm.out2, id = "ensemble")

cat("Threshold usado para mapas binários:", thresholds, "\n")

# 3. FUNÇÃO PARA PROCESSAR CADA CENÁRIO (CORRIGIDA) ---------------------------

process_single_scenario <- function(scenario_file, scenario_name, pca_metadata, thresholds) {
  
  cat("\n", paste(rep("=", 60), collapse = ""), "\n")
  cat("Processando:", scenario_name, "\n")
  cat("Arquivo:", basename(scenario_file), "\n")
  
  # 3.1 Carregar dados futuros
  future_predictors <- rast(scenario_file)
  
  # 3.2 Aplicar PCA
  future_pc12 <- apply_pca_to_future(future_predictors, pca_metadata)
  
  # 3.3 Projeção do modelo
  ensemble_future <- ensemble(
    my.sdm.out2, 
    newdata = future_pc12,
    filename = paste0("Trips_palmi_", scenario_name, "_ensemble.tif"),
    setting = list(method = 'weighted', stat = 'AUC', opt = 2),
    overwrite = TRUE
  )
  
  # 3.4 Mapa binário
  binary_future <- ensemble_future > thresholds
  writeRaster(binary_future, 
              paste0("Trips_palmi_", scenario_name, "_binary.tif"),
              overwrite = TRUE)
  
  # 3.5 Mapa de mudanças vs atual
  change_map <- ensemble_future - ensemble_global
  writeRaster(change_map,
              paste0("Trips_palmi_", scenario_name, "_change.tif"),
              overwrite = TRUE)
  
  # 3.6 Estatísticas
  current_suitable <- sum(values(binary_map), na.rm = TRUE)
  future_suitable <- sum(values(binary_future), na.rm = TRUE)
  absolute_change <- future_suitable - current_suitable
  percent_change <- (absolute_change / current_suitable) * 100
  
  # 3.7 Retornar resultados
  return(data.frame(
    scenario = scenario_name,
    model = scenarios_metadata$model[scenarios_metadata$scenario_name == scenario_name],
    ssp = scenarios_metadata$ssp[scenarios_metadata$scenario_name == scenario_name],
    period = scenarios_metadata$period[scenarios_metadata$scenario_name == scenario_name],
    current_area = current_suitable,
    future_area = future_suitable,
    absolute_change = absolute_change,
    percent_change = percent_change
  ))
}

# 3b. FUNÇÃO apply_pca_to_future (CASO NÃO EXISTA) ----------------------------

if(!exists("apply_pca_to_future")) {
  apply_pca_to_future <- function(future_data, pca_metadata) {
    
    # Extrair estatísticas de padronização da PCA original
    center_vals <- pca_metadata$scaling_center
    scale_vals <- pca_metadata$scaling_scale
    rotation_matrix <- pca_metadata$loadings
    
    # Função para aplicar PCA
    pca_fun <- function(x) {
      # Padronizar com os MESMOS parâmetros da PCA original
      x_scaled <- scale(x, center = center_vals, scale = scale_vals)
      
      # Substituir NAs por 0 (caso ocorram)
      x_scaled[is.na(x_scaled)] <- 0
      
      # Aplicar transformação PCA
      pca_scores <- x_scaled %*% rotation_matrix
      
      return(pca_scores[, 1:2])  # Manter apenas PC1 e PC2
    }
    
    # Aplicar ao raster futuro
    future_pc <- app(future_data, pca_fun)
    names(future_pc) <- c("PC1", "PC2")
    
    return(future_pc)
  }
}

# 4. EXECUTAR PROCESSAMENTO PARA TODOS OS CENÁRIOS (CORRIGIDO) ----------------

# Lista para armazenar resultados
all_results <- list()

# Verificar se objetos necessários existem
required_objects <- c("ensemble_global", "binary_map", "my.sdm.out2")
missing_objects <- required_objects[!sapply(required_objects, exists)]

if(length(missing_objects) > 0) {
  cat("❌ Objetos faltantes:", paste(missing_objects, collapse = ", "), "\n")
  cat("Carregue os objetos necessários antes de continuar.\n")
} else {
  cat("✅ Todos os objetos necessários estão disponíveis\n")
  
  # Processar cada cenário
  for(i in 1:nrow(scenarios_metadata)) {
    scenario_file <- scenarios_metadata$file_path[i]
    scenario_name <- scenarios_metadata$scenario_name[i]
    
    # Verificar se arquivo existe
    if(file.exists(scenario_file)) {
      cat("Processando", i, "de", nrow(scenarios_metadata), "-", scenario_name, "\n")
      result <- process_single_scenario(scenario_file, scenario_name, pca_metadata, thresholds)
      all_results[[i]] <- result
      cat("✅ Concluído:", scenario_name, "\n")
    } else {
      cat("❌ Arquivo não encontrado:", scenario_file, "\n")
    }
  }
  
  # 5. COMBINAR RESULTADOS E GERAR RELATÓRIOS ---------------------------------
  
  if(length(all_results) > 0) {
    # Combinar todos os resultados
    results_df <- do.call(rbind, all_results)
    
    # Ordenar por percentual de mudança
    results_df <- results_df[order(results_df$percent_change), ]
    
    print("=== RESULTADOS COMPARATIVOS ===")
    print(results_df)
    
    # Salvar tabela completa de resultados
    write.csv(results_df, "Trips_palmi_all_scenarios_comparison.csv", row.names = FALSE)
    ...
    write.csv(summary_report, "Trips_palmi_scenarios_summary.csv", row.names = FALSE)
    
    cat("\n", paste(rep("=", 60), collapse = ""), "\n")
    cat("✅ PROCESSAMENTO CONCLUÍDO!\n")
    cat("📊 Total de cenários processados:", nrow(results_df), "/", nrow(scenarios_metadata), "\n")
    cat("💾 Resultados salvos em:\n")
    cat("   - Trips_palmi_all_scenarios_comparison.csv\n")
    cat("   - Trips_palmi_scenarios_summary.csv\n")
    cat("   - Mapas individuais para cada cenário\n")
    
  } else {
    cat("❌ Nenhum cenário foi processado com sucesso.\n")
  }
}
